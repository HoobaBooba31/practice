...
def f(x1, x2):
    return -(
        np.sin(x1) * np.sin(x1**2 / pi) ** 20
        + np.sin(x2) * np.sin(2 * x2**2 / pi) ** 20
    )
...
def main():
    ...
    ax1.scatter(
        cord[0], cord[1], f(cord[0], cord[1]), color="red", s=50, label="Тестовая точка"
    )
    ax1.text(
        cord[0],
        cord[1],
        f(cord[0], cord[1]),
        f"({cord[0]}, {cord[1]}, {f(cord[0], cord[1]):0.2f})",
        color="black",
    )
